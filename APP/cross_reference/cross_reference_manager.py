...
class CrossReferenceManager:
	...
	def set_new_cross_reference(self, cross):
		...
		with self.lock1: # 暫定履歴交差ブロックLock
			logging.debug("self.lock1_ON-set_new_cross_reference == 暫定履歴交差ブロックLock")
			self.reference.append(cross)
			print("======= set_new_cross_ref =======")
		logging.debug("self.lock1_OFF-set_new_cross_reference == 暫定履歴交差ブロックLock")

	def store_cross_reference(self):
		logging.debug("rq == self.lock1-2")
		with self.lock1: # 暫定履歴交差ブロックLock
			logging.debug("self.lock1_ON-store_cross_reference == 暫定履歴交差ブロックLock")
			print("store_cross_reference")
			self.set_new_cross_reference(self.cross_reference)
		logging.debug("self.lock1_OFF-store_cross_reference == 暫定履歴交差ブロックLock")
	...
	def add_cross_reference(self, cross): #reference部形成リスト
		...
		with self.lock1:# 暫定履歴交差ブロックLock
			logging.debug("self.lock1_ON-add_cross_reference == 暫定履歴交差ブロックLock")
			print("add_cross_reference")
			self.cross_reference.append(cross)
		logging.debug("self.lock1_OFF-add_cross_reference == 暫定履歴交差ブロックLock")
			

	def clear_cross_reference(self):
		logging.debug("rq == self.lock1-4")
		with self.lock1:# 暫定履歴交差ブロックLock
			logging.debug("self.lock1_ON-clear_cross_reference == 暫定履歴交差ブロックLock")
	...
	def get_reference_pool(self):
		...
		with self.lock3:
			...
			elif len(self.reference) > 1: # ここは通らなくなった。
				logging.debug("len(self.reference) > 1")
	...
	def store_previous_cross_ref(self, msg_sig):
		with self.lock2: #previous_履歴交差法ブロックhash
